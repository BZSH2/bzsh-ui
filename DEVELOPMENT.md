...
如果希望自动创建指定类型的 changeset，也可以直接附带版本类型和说明：

```bash
pnpm ship patch "release summary"
```
...
#### 3. 等待 CI 自动创建 Release 并发布到 npm

版本提交推送成功后：

- [auto-release.yml](./.github/workflows/auto-release.yml) 会自动创建 GitHub Release
- [publish-npm.yml](./.github/workflows/publish-npm.yml) 会继续执行发布流程
